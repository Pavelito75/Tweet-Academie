...
<body class="dark:bg-neutral-900 dark:text-neutral-300">
    ...
    <main class="container mx-auto mt-6 grid grid-cols-12 gap-6 px-6">
        ...
        <nav id="left" class="col-span-3 flex flex-col justify-between">
            <div>
                ...
                <ul class="flex flex-col gap-9 text-2xl select-none">
                    <a href="/feed" class="flex gap-6 items-center cursor-pointer hover:bg-neutral-800 rounded-full p-3 pl-6">
                        <img src="/assets/Icons/home.png" alt="Icone Home" class="w-8 h-8">
                        <p class="h-fit font-bold text-white">Accueil</p>
                    </a>
                    <a href="/feed" class="flex gap-6 items-center cursor-pointer hover:bg-neutral-800 rounded-full p-3 pl-6">
                        <img src="/assets/Icons/search.png" alt="Search Home" class="w-8 h-8">
                        <p class="h-fit">Explorer</p>
                    </a>
                    <a href="/messages" class="flex gap-6 items-center cursor-pointer hover:bg-neutral-800 rounded-full p-3 pl-6">
                        <img src="/assets/Icons/answer.png" alt="Message Home" class="w-8 h-8">
                        <p class="h-fit">Message</p>
                    </a>
                    <a href="/korg" class="flex gap-6 items-center cursor-pointer hover:bg-neutral-800 rounded-full p-3 pl-6">
                        <img src="/assets/Icons/korg.png" alt="Korg Home" class="w-8 h-8">
                        <p class="h-fit">Korg</p>
                    </a>
                    <a href="/account" class="flex gap-6 items-center cursor-pointer hover:bg-neutral-800 rounded-full p-3 pl-6">
        ...
        <!-- Section centrale -->
        <section class="col-span-6 ml-8 bg-neutral-800 rounded-2xl shadow-lg p-6 border-l border-r border-neutral-600">
            ...
            <section class="bg-neutral-800 p-6 rounded-lg shadow">
                <div class="flex items-center space-x-6">
                    ...
                    <div>
                        ...
                        <p class="text-neutral-400">@<?= htmlspecialchars($user['username'] ?? '') ?></p>
                        ...
                        <div class="mt-4 flex space-x-6">
                            <span><span class="text-white"><?= $followersCount ?? 0 ?></span> Abonnés</span>
                            <span><span class="text-white"><?= $followingCount ?? 0 ?></span> Abonnements</span>
            ...
            <?php else: ?>
                <p class="text-neutral-400">Aucun profil utilisateur à afficher.</p>
            <?php endif; ?>

            <!-- Formulaire de tweet -->
            <section class="bg-neutral-800 p-6 mt-6 rounded-lg shadow">
                <form action="/account/update" method="POST" enctype="multipart/form-data">
                    <textarea name="content" class="w-full p-4 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-300 bg-neutral-700 text-white" rows="3" placeholder="Exprimez-vous..."></textarea>
            ...
            <section class="mt-6">
                <?php if (isset($tweets) && !empty($tweets)): ?>
                    ...
                    <article class="bg-neutral-800 p-4 rounded-lg shadow hover:bg-neutral-700 transition-all duration-200 mb-4">
                        <div class="flex items-start space-x-4">
                            ...
                            <div>
                                <header class="flex items-center space-x-2">
                                    ...
                                    <span class="text-neutral-400">@<?= htmlspecialchars($tweet['username'] ?? 'utilisateur') ?></span>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <p class="text-neutral-400">Aucun tweet à afficher.</p>
                <?php endif; ?>
            </section>
        </section>
    </main>
    ...
    <div class="fixed inset-0 flex items-center justify-center hidden" id="editProfilePopup">
        <div class="bg-neutral-800 p-6 rounded-lg shadow-lg w-full max-w-md">
            <form action="/account/update" method="POST" enctype="multipart/form-data">
                <h2 class="text-xl font-bold mb-4">Éditer le profil</h2>
                <div class="mb-4">
                    <label for="firstname" class="block text-sm font-medium">Prénom</label>
                    <input type="text" id="firstname" name="firstname" value="<?= htmlspecialchars($user['firstname'] ?? '') ?>" class="w-full p-2 border rounded-lg bg-neutral-700 text-white">
                </div>
                <div class="mb-4">
                    <label for="lastname" class="block text-sm font-medium">Nom</label>
                    <input type="text" id="lastname" name="lastname" value="<?= htmlspecialchars($user['lastname'] ?? '') ?>" class="w-full p-2 border rounded-lg bg-neutral-700 text-white">
                </div>
                <div class="mb-4">
                    <label for="username" class="block text-sm font-medium">Nom d'utilisateur</label>
                    <input type="text" id="username" name="username" value="<?= htmlspecialchars($user['username'] ?? '') ?>" class="w-full p-2 border rounded-lg bg-neutral-700 text-white">
                </div>
                <div class="mb-4">
                    <label for="biography" class="block text-sm font-medium">Biographie</label>
                    <textarea id="biography" name="biography" class="w-full p-2 border rounded-lg bg-neutral-700 text-white"><?= htmlspecialchars($user['biography'] ?? '') ?></textarea>
                </div>
                <div class="mb-4">
                    <label for="picture" class="block text-sm font-medium">Photo de profil</label>
                    <input type="file" id="picture" name="picture" class="w-full p-2 border rounded-lg bg-neutral-700 text-white">
                </div>
                <div class="mb-4">
                    <label for="city" class="block text-sm font-medium">Ville</label>
                    <input type="text" id="city" name="city" value="<?= htmlspecialchars($user['city'] ?? '') ?>" class="w-full p-2 border rounded-lg bg-neutral-700 text-white">
                </div>
                <div class="mb-4">
                    <label for="country" class="block text-sm font-medium">Pays</label>
                    <input type="text" id="country" name="country" value="<?= htmlspecialchars($user['country'] ?? '') ?>" class="w-full p-2 border rounded-lg bg-neutral-700 text-white">
                </div>
